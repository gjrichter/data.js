...
$(function() {
	...
	var myfeed = Data.feed("Segnalazioni",{"source":szUrl,"type":"csv"}).load(function(mydata){
		...
		var list = "";
		for (i=0; i<100; i++ ){
			list += '<a href="'+mydata.records[i][0]+'" target=_blank">';
			list += '<li>';
			list += '<i class="icon fa '+__getIcon(mydata.records[i][7])+' fa-2x pull-left" ></i>';
			list += '<div class="message-block">';
			list += '<div><span class="username">'+mydata.records[i][3]+'</span> <span class="message-datetime">'+mydata.records[i][7]+'</span>';
			list += '</div>';
			list += '<div class="message">'+mydata.records[i][4]+'</div>';
			list += '</div>';
			list += '</li>';
			list += '</a>';
		}
		...
		mydata = mydata.addColumn({'source':'created_at','destination':'hour'},
								   function(value){
										var d = new Date(__normalizeTime(value));
										return( d.getHours() );
										} 
